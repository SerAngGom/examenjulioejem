...
function createPlatform(element) {
    ...
    let platform = platforms.create(element.x, game.world.height - element.y, 'ground');
    platform.body.immovable = true;
    
    for (let i = 0, max = element.enemies.length; i < max; i++)
        setupEnemy(element.enemies[i], platform);
    
    for (let i = 0, max = element.aids.length; i < max; i++)
        setupAid(element.aids[i], platform.y);

    for (let i = 0, max = element.stars.length; i < max; i++)
        setupStar(element.stars[i], platform.y);
}
