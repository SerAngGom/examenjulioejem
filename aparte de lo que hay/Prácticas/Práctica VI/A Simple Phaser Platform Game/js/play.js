...
function createLevel() {
    ...
    player = game.add.sprite(levelConfig.collectorStart.x, game.world.height - levelConfig.collectorStart.y, 'collector');
...
function createPlatform() {
    plat = platforms.create(levelConfig.platformData[aux].x, levelConfig.platformData[aux].y, 'ground');
    plat.scale.setTo(1, 1); // 400x32 ---> 1100x64
    plat.body.immovable = true;

    for (let i = 0, max = levelConfig.platformData[aux].enemies.length; i < max; i++)
        setupEnemy(levelConfig.platformData[aux].enemies[i], plat);

    for (let i = 0, max = levelConfig.platformData[aux].aids.length; i < max; i++)
        setupAid(levelConfig.platformData[aux].aids[i], plat.y);

    for (let i = 0, max = levelConfig.platformData[aux].stars.length; i < max; i++)
        setupStar(levelConfig.platformData[aux].stars[i], plat.y);
...
function debugGame() {
    ...
    game.debug.physicsGroup(platforms,'rgba(255,0,0,0.5)', false);
}